...
> 可直接复制本文件给下一位 agent。更新每次会话结束/轮次切换时。本版更新于 2026-09-08（第三十三轮：小红书三功能真实体验图文——PDF 合并 / 房贷计算 / 二维码生成 实拍素材 + 体验文案 + 产物程序核验登记）。

## 〇、2026-09-08 第三十三轮（小红书三功能真实体验图文：PDF 合并/房贷计算/二维码生成 真跑 + 实拍素材 + 文案，本轮）

- **任务（用户）**：模拟真实用户使用 ZonKey 体验多种功能（PDF 工坊、计算工具等），选三个功能写小红书分享文案并生成对应功能的使用素材，贴近真实使用后的体验。
- **功能选型**：PDF 工坊「PDF 合并」+ 计算开发「房贷计算」「二维码生成」（跨两大中心、办公/置业/日常三个真实场景；UI 全 DOM 探查后定）。
- **实现**：
  - **真跑当前版 UI 全流程**（uvicorn 8765 + vite 5173 + Playwright，全程零 pageerror）：① 合并：合成「智能温控器说明书 上/下篇」（各 2 页，纯通用内容）一次拖入 → 按列表顺序合并 → 任务完成弹窗 → 下载产物；② 房贷：100 万/3.1%/30 年本息 → 改 20 年 → 切等额本金，三组 UI 实算；③ 二维码：备忘文字 → 生成 → 下载产物 PNG。
  - **产物程序核验（文案数字全有实证）**：合并产物 `merged_20260908.pdf` 4 页顺序零乱序（1-2=上篇 3-4=下篇）；房贷本息 30 年 4,270.16/537,259.04、本息 20 年 5,596.17/343,080.55、本金 30 年首月 5,361.11/末月 2,784.95/466,291.67（20 年 vs 30 年利息差 19.4 万、本金省 7.1 万）；二维码 cv2 解码 == 输入文字（一字不差）。
  - **素材全为真实画面**：`docs/screenshots/round33/raw/` 12 张（merge×3、loan×4、qr×2、合并产物 p1/p3 200dpi 渲染、qr 产物原图，Playwright 2×DSF）；`scripts/gen_xhs_r33.py` → `docs/screenshots/round33/xhs/r33-01~08.png`（1242×1656 白底素人风：封面 + ①合并② 卡→②房贷② 卡→③二维码② 卡→三个真实提醒收尾），内置 BANNED 零导流校验（按 r33 放宽「二维码/网址」词面——工具名词，URL 形态另行拒绝）。
  - **文案**：MOMENTS_KIT.md 新增 round-33 节（标题×3 ≤20 字 / 第一人称正文三件事含 UI 实算数字与三个真实提醒 / 话题 / 8 图顺序表 / 发布建议：与 r31/r32 错开 24-48h、07 卡二维码为纯文字备忘非外链、若平台误判有 loan-03 换卡备选方案与卡面文案）。
- **验证汇总**：卡面文字逐条宽度断言零溢出 + 像素四边距审计 8/8（≥40px）+ 截图语义与 DOM 拍摄互证（judge/Read 图像通道本会话仍不可用，r30/r32 同况降级协议）；产物三组证据见上。
- **真实使用观察（非缺陷，登记待决策）**：① 合并列表行不显示各文件页数；② 合并产物命名 `merged_YYYYMMDD.pdf` 不继承来源名。均可选改进，未改码。
- **收尾状态**：git 分批提交 master（脚本/素材/文档）。纯素材+文档轮：无需重打包 EXE / 部署 Pages / 升 WHATSNEW_ROUND。
- **接手注意**：① 8 卡发布前建议人工目检一次（本会话无视觉通道）；② r32 遗留 F1/F2 仍待用户拍板是否修复；③ 三篇小红书笔记（r31 官方更新总览 / r32 图纸脱敏体验 / r33 三功能实测）互相独立，发布间隔 ≥24-48h；④ `temp_ui_test/r33_*`（r33_probe/flow/fixtures + r33_mat/r33_out + _r33_font_probe.*）为未跟踪临时产物，可清可留；⑤ r33 演示样张=合成说明书，若换真实材料重拍，文案与卡面数字需同步改；⑥ 素材曾因 promo 条重拍——temp_ui_test 脚本的 add_init_script 种子**必须写成 IIFE（`(() => {...})()`）**，纯箭头函数表达式只会被求值不执行，localStorage 永远写不进（r32/r33 旧写法均踩过）。

## 〇、2026-09-08 第三十二轮（小红书「真实用户体验」向图文：真实跑流程 + 实拍素材 + 体验文案，本轮）

- **任务（用户）**：模拟真实用户使用 ZonKey 写小红书分享文案、生成使用素材，要贴近真实用户使用后的体验。
- **实现**：
  - **真跑当前版 UI 全流程**（uvicorn 8765 + vite 5173 + Playwright，全部真实操作）：首页 → 智能脱敏 → 拖入合成图纸（`sample_drawing.pdf` 更名「外发加工图-法兰盘D3_v4.pdf」，通用保密词样张，无客户文件）→ 自动识别 5 处命中 → 执行脱敏 → 「脱敏前/脱敏后」对比视图 → 导出 `_desensitized.pdf`。API 链：upload-preview → scan-candidates → update-candidate-boxes → execute-redaction。
  - **真实证据链**：全选后执行导出 = 全文检索零命中 + 框外内容保留；**未全选直接执行 = 残留 2 处**（CONFIDENTIAL/INTERNAL USE，见 `temp_ui_test/r32_dl/diag_no_select.pdf`）——该对比成了体验文案里「先全选再执行」提醒的实证，不是编的。
  - **素材全为真实画面**：`docs/screenshots/round32/raw/`（UI 实拍 5 张 2×DSF + 产物渲染裁剪 before/after_notes·title @300dpi）；`scripts/gen_xhs_r32.py` 组版 → `docs/screenshots/round32/xhs/r32-01~07.png`（1242×1656 白底素人风：封面+①首页→②拖入→③命中全选→④抹前/抹后对比→⑤导出→⑥规则词表），内置 BANNED 零导流校验同 r31。
  - **文案**：MOMENTS_KIT.md 新增 round-32 节（标题×3 ≤20 字 / 第一人称体验正文 / 话题 / 配图顺序表 / 发布建议：与 r31 篇错开 ≥24h、r31 五条纪律沿用、素材为合成样张勿对外）。
  - **真实使用发现（PROJECT_STATUS r32 节 F1/F2，未改代码）**：F1=「执行脱敏」只处理已纳入命中、默认仅部分自动纳入且无未纳入提示 → 直点执行+导出带残留（泄密风险口子）；F2=标题栏同行文本带整行连坐（MARKING/SHEET/1 OF 1 非敏感标签与 CONFIDENTIAL 同行被清，行级像素剖面实证 y≈792–803pt/x≈700–1064pt），疑似检测盒同行 span 合并过宽、与内部版收缩规则预期不符，涉共享 core，修须走内部版回归。两发现均建议立项，等用户决策。
- **验证汇总**：零命中/保留证据（fitz 程序化）；卡面文字逐条宽度断言 + 像素边距/层次审计全绿（judge/Read 图像通道本会话不可用，r30 同况降级协议）；7 卡几何复盘一处修复（对比胶囊左上也压 NOTES 标题 → 锚右上留白区）。
- **收尾状态**：git 分批提交 master（脚本/素材/文档）。纯素材+文档轮：无需重打包 EXE / 部署 Pages / 升 WHATSNEW_ROUND。
- **接手注意**：① F1/F2 待用户拍板是否修（F2 动 core 先内部版回归）；② `temp_ui_test/r32_*`（probe/shoot/export/semantics/diag/semantics 脚本 + r32_shots/r32_dl + `_r32_外发加工图-法兰盘D3_v4.pdf`）为未跟踪临时产物，可清可留（shots_full 既有先例）；③ r31 官方口径卡与 r32 体验图文是两篇笔记，均零导流纪律，勿混发；④ 发布素材若换真实图纸重拍，「5 处」等文案数字要同步改。

## 〇、2026-09-08 第三十一轮（小红书限流重做：零导流图卡 + 文案，上轮）

- **任务（用户反馈）**：r29 小红书笔记 2026-09-07 20:47 发布、20:57 即被限流，审核详情「**笔记存在外部平台信息或联系方式**」。要求重新准备文案与素材。触发点：图卡底部域名、xhs-04 二维码与「GitHub/Gitee 搜 ZonKey Release 下载」、正文末行链接、评论区置顶链接——全部属站外导流。
- **实现**：
  - `scripts/gen_xhs_r31.py`（上一会话遗留未跑，本会话跑通 + 返工）：小红书 4 卡**零导流重做**——无网址/域名/二维码/「扫一扫」，无 GitHub/Gitee/微信/QQ/邮箱等渠道名，无「评论区自取/私信」句式；底栏改页码点+左滑提示；色斑方位与 r29 错开防同图重复判定；内置 BANNED 红线词校验（图卡文本命中任何站外信息拒绝出图）。输出 `docs/screenshots/round31/xhs/xhs-01~04.png`（1242×1656, 3:4；01 封面→02 清单→03 认识→04 获取）。内容口径与弹窗 whatsnew r30 同源不变（自首版以来更新总览 4 主线）。
  - **judge 首轮 3/4 pass**，xhs-01 两处 fail 返工：① `app_home` 手机示意六宫格瓦片高仅 41px 塞三层内容（名称/小字/底部指示条互相压叠）→ 改 **2×2 宽卡瓦片**（左色点+名称+小字，瓦片 ~76px；工具收为 4 款高频：二维码/证件照/图片打码/PDF 批处理）+ mockup 加高 494→540；② `wrap` 字符级断行把「70+」劈成「7」/「0+」→ 改 **ASCII 连续串整体断行**（数字/英文/版本号如 70+、v2.2.0、diff 不再劈开）。复检 4/4 pass。
  - **MOMENTS_KIT.md**：新增 **round-31 节**——标题×3（均 ≤20 字）+ 正文（零链接零渠道名，末尾不放任何链接）+ 话题标签 + **限流重发版发布建议**（旧笔记先删隔 24–48h 再发 / 全链路零导流 / 转化只提品牌词「搜 ZonKey 就行」/ 网址只放个人主页简介）；r29 小红书节加盖废弃警告（勿再用其文案与图卡）。
- **验证汇总**：脚本红线校验 + xhs-02 溢出断言通过；judge 视觉验收 **4/4 pass**（唯一返工页 xhs-01 复检通过）。
- **收尾状态**：git 分批提交 master（脚本/素材/文档）。纯素材+文档轮：无代码改动，无需重打包 EXE / 部署 Pages / 升 WHATSNEW_ROUND。
- **接手注意**：① 小红书物料从此有**硬性零导流纪律**（图卡+正文+评论区一体适用），改文案先对齐 `gen_xhs_r31.py` 的 BANNED 列表口径；② 朋友圈/抖音文案仍带链接（r29 口径）未动——该两平台未触发限流，若未来也要零导流再立轮；③ r29 小红书 4 卡（`docs/screenshots/round29/xhs/`）保留为历史档案**勿重发**；④ 转化路径收窄为「品牌词搜索 + 主页简介」，xhs-04 卡「搜索 ZonKey 就能找到」是有意保留的 brand-only 引导（无平台名无链接，本轮审核口径下安全）。

## 〇、2026-09-07 第三十轮（弹窗口径重做：首版至今更新总览 + 素材重做 2，上轮）

- **任务（用户反馈）**：r29 弹窗/素材把「打开预览/零上传/下载收口」写成 v2.2.0 本次更新卖点 → 用户纠正：**「打开/直接预览」首版即有能力，不是更新**；弹窗内容应写**从首版发布到现在的更新内容**（如多种小工具、二维码生成、证件照生成等）。要求：重写软件弹窗文案 + 重新生成三平台素材。
- **实现**：
  - **i18n whatsnew 三语重写**（`zh-CN.ts`/`zh-TW.ts`/`en.ts`）：r26–r29 四条旧轮条目整体删除（含「打开直接预览/零上传/要存档再点下载」r29 三条与 r26-r28 旧轮混编条目），改为 **r30 单条总览**——badge「自首版发布以来」（en Since the first release）、date 2026-09-07，i1–i4：①10 款高频小工具（二维码生成/识别、证件照换底色与裁剪、图片打码、文本 diff、正则测试、批量重命名、重复文件查找、PDF 书签编辑、文字朗读、单位/进制换算）②PDF/图像/PPT 批处理 ③「任务完成」弹窗 ④手机网页版同步；`whatsnew.title` zh「这次更新了什么」→「更新内容」（zh-TW 更新內容；en What's New 原样），desc 点明「从首版发布（v1.0）到现在」。弹窗因旧轮键删除只渲染 r30 一节。
  - **WhatsNewModal**：WHATSNEW_ROUND 29→30；**触发语义修正**——删除「seenRound==ROUND-1 且 lastSeenDay==今天不弹」的同天抑制（同日多轮发布 r28→r29→r30 会吞新内容：今早看过 r29 的用户看不到 r30；与「改内容必升轮、升轮必可见」冲突），现规则 `seenRound < WHATSNEW_ROUND` 一律弹一次；isToday() 删除。头注释同步。
  - **素材重做 2**（`scripts/gen_social_r29.py` 整体重写后重跑，输出覆盖 `docs/screenshots/round29/` 10 卡）：口径与弹窗 r30 同源。moments-01 主卡：「更新总览 · 自首版发布以来」pill + 「从首版发布到现在 / 一直在上新（马克笔高亮）」+ 两行简介 + ×第一版/√现在对比小卡 + 右侧**手机示意=应用首页·高频小工具墙**（app_home()：状态栏+搜索条「搜索 70+ 工具」+六宫格 二维码/证件照/图片打码/PDF 批处理/文本 diff/单位换算 +底部导航首页/工具/我的——替换旧 phone()「转换完成→打开/下载」弹窗 mock，打开预览非新卖点不可入画）+ 底条 QR/三 pills/域名；moments-02 清单卡：双 pills「更新总览 / v1.0 → v2.2.0」+ 大字「更新了什么」+ **自适应高度 4 条编号清单**（updates_list 重写：按 wrap 行数自动定高，白字圆点编号）+ 脚注 + QR；moments-03 获取卡（网页版卡片行改「最新 v2.2.0 已上线：软件内有完整更新说明」）。xhs-01 封面（「从首版发布到现在 / 一直在上新」大字+长简介+三 pills+app_home），xhs-02 清单（域名底部居中 1552），xhs-03 认识卡（底部改「以上更新已全部上线 · 完整清单见软件「更新内容」弹窗」），xhs-04 获取；douyin-01 封面（fs100/116 大字+app_home h700），douyin-02 清单（条目加大 pad30/fs48/31/34 + **底部补「免费开源·无广告·不注册不登录」胶囊+域名 1812，与 01/03 家族一致**），douyin-03 获取。
  - **MOMENTS_KIT.md round-29 节重写（重做 2）**：口径注「与 App 弹窗 r30 同源 4 主线」；朋友圈文案 A（更新通报型，首版=智能脱敏工作台→今天 8 大中心 70+）/B（简短型）；小红书标题三选一+正文；抖音文案+口播脚本（约 25s）+发布建议；旧口径短语零残留（grep 断言过）。
- **验证汇总**：tsc 零错 + npm build（入口 `index-CtQErp5F.js` / legacy `index-legacy-DlNXoTjP.js`；「更新内容/自首版发布以来」在包；'第 29 轮' 零命中；'直接预览' 仅剩 taskDone.hint 操作提示属正常 UI 非弹窗残留）；新 `temp_ui_test/r30_whatsnew_smoke.py` **6/6**：A zh-CN 全新（隐私→弹窗→标题/徽标/desc/4 条→**section 数=1**→旧口径零残留→知道了落盘 30→刷新不弹）/ **B seenRound=29+今天看过→仍弹 r30**（关键回归点）/ C seenRound=30 不弹 / D zh-TW / E en / F 390px 弹窗不超视口零溢出，全程零 pageerror。**素材像素边缘审计 10/10**：内容距画布边 ≥37px，零溢出/截断；清单页底部留白收匀（douyin 家族 B=57 一致）。
- **收尾状态**：git 分批提交 master（代码+素材+文档）。**EXE 重打包 / Pages 部署 / Release 未执行**：v2.2.0（0907 批次）资产仍最新，本轮纯内容修正；素材可直接发布，弹窗 r30 随下次收尾链自动进包上线。本地 dist_web 已重建、壳内可验收新弹窗。
- **接手注意**：① 触发语义已简化——**同天低轮也弹一次**，r22 冒烟 case「上轮当天已看过不弹」已过时勿回退；② 弹窗与素材同口径，改任意一边须同步 i18n whatsnew r30 与 gen_social_r29.py 的 UPDATES/COVER 文案（真源=弹窗条目）；③ judge 视觉通道本会话不可用（PNG Read 返回 Unsupported Image），素材验收降级=像素边缘审计+布局几何复核，有视觉通道的会话建议补一次目检；④ r30 badge 无轮次数字（「自首版发布以来」）是口径选择非遗漏；⑤ 发布前跑一次 `python scripts/gen_social_r29.py` 确认 10 卡 mtime 全刷（素材与弹窗文案同批）。
...
- **朋友圈物料**：`docs/screenshots/round29/moments/moments-01~03.png`（1080×1440 3:4，主卡/更新清单/获取方式）+ `scripts/gen_moments_r29.py`（PIL 生成，品牌色与 index.css 同源，皇冠 alpha_composite）+ MOMENTS_KIT.md round-29 节（文案 A/B + 发布建议）。
  - **当晚重做（用户反馈：写清楚「本次更新了什么」）**：初版把 r26-r28 旧功能（QR/证件照）混排成「这次更新」清单——用户纠正本次只有两条核心变化：①「打开」弹窗内直接预览（本机渲染零上传）②不再自动下载。`gen_moments_r29.py` 文案结构重写：卡1=一句话点题+×/√ 以前现在对比条；卡2=「本次更新了什么」3 件事清单（全部为本次变化，桌面端「仍走系统默认程序」脚注级区分）；卡3=获取方式不变。MOMENTS_KIT.md 文案 A/B 同步（旧功能降为一行脚注）。三卡 judge 视觉验收 3/3 pass。
  - **Release 状态核实 → v2.2.0 已发布（用户确认）**：线上原 Latest=v2.1.0（资产 20260903 批次）落后本地 20260907；用户选「新建 Release」。执行：红线审计（企业词仅命中 release_acceptance 防御断言）→ push master（`25dede5..f660551`）→ tag v2.2.0（gh release create 直建 tag 报 422，须本地 tag+push tag 再建 Release）→ 6 资产（Setup/7z/zip+sha256×3）上传核验字节一致，v2.2.0 即 Latest——**网页下载弹窗自此拉到 0907 新版**；macOS DMG 由 Actions tag 触发自动附加（后台任务 cwd 与前台持久化 cwd 不一致，首次 `cd dist_release` 落空失败，绝对路径/确认 cwd 后重跑成功）。
  - **Gitee 镜像同步 → 策略变更（用户拍板）**：Gitee 单附件 **100MB 硬顶**（实测 HTTP 400「文件大小已超出限制：100 MB」，v1.0.1 的 zsplit_aa/ab/ac 分卷即此因）；本轮曾把 0907 三产物切 95MiB 分卷（7 卷）+sha256 传上 Gitee Release 并本地合并校验 3/3 过——**随后用户定调：ghfast.top 加速已够国内下载，Gitee 只保留代码镜像仓库角色，桌面版二进制一律不进 Gitee Release**。已删净 10 个附件（attach_files 子接口拿 id 逐个 DELETE；release 列表接口的 assets 无 id），Release 页保留为纯更新日志+下载指引（网页版弹窗/GitHub Release）。前端核查：仅 brand.ts `PROJECT_GITEE_URL` 一处 URL，三处用法（SupportAuthorModal/DownloadPromo/HelpModal）均为「仓库镜像」链接，无下载链，无需改码。**教训：后台任务与跨调用路径一律绝对路径**（cwd 在 dist_release/gitee_split 等处漂移已两次翻车）。
...
- **验证汇总**：npm build 成功（主 chunk `index-Bp1zXvkA.js`，dist_web 已刷新）；pytest **141 passed**（--ignore native dialog）；`toolbox_smoke.mjs` **10/10 零 pageerror**；新 `temp_ui_test/r24_mask_pointer_smoke.mjs` **9/9**（鼠标 pointer 框选成区 / CDP touch 事件链触屏框选成区（探针实证 pointerType=touch）/ 手机 390px 零 pageerror / 预览存在 / 一寸 49×69 渲染 / 比例 25/35=0.714 vs 33/48=0.687 断言 / 二寸>一寸同比例尺）；后端合成证件照双场景（纯色蓝底 295×413 白底合成 + 条纹干扰底不漏）TestClient 实测；**桌面壳内 SendInput 真鼠标拖拽框选→已选 1 区域→应用打码→保存图片出现**全链路 + 壳内尺寸预览渲染确认。**Pages 生产部署 `986ce016`**（--branch main，Production 实证，Source=8324bb5）：主域 bundle=`index-Bp1zXvkA.js`，线上 md5=本地，round-24 特征串（touch-none/setPointerCapture/mm @ 300DPI）全命中。
- **接手注意**：① 触屏冒烟坑：CDP `dispatchTouchEvent` 的坐标要落图片**内部**（手机 390px 视口图片常在首屏外，先 `scrollIntoViewIfNeeded` 再取 box）；② 桌面壳 computer-use 拖拽在 WebView2 表面 frame 绑定持续失配（渲染面属 msedgewebview2 子进程无 UIA 树、背景动画致 full-display raster 恒 stale）——**绕法=Python ctypes mouse_event SendInput 按窗口 rect 换算屏幕坐标真拖**，反而是最真实的触点验证；③ 证件照回退触发阈值（<2%/>90%）比旧版（<5%/>95%）收紧，GrabCut 只在色距彻底失效时跑；④ Pages 已部署（986ce016）；**EXE 重打包本轮未跑**——dist_web 已就绪，下轮收尾链或用户要求时执行 `build_zonkey_exe.bat`。
