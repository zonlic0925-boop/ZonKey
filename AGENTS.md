...
**ZonKey**（by zonlic）：本地离线脱敏工作台，面向公开发布与通用客户场景。
...
| 工作区 | 路径 | 用途 |
| --- | --- | --- |
| **ZonKey（本仓库，本地目录 Desktop/ZonScale，GitHub 仓库 zonlic0925-boop/ZonKey）** | `Desktop/ZonScale` | 公开发布版，**禁止**内置 Fisher/Emerson/TopWorx/MKS 等企业规则与 Logo |
...
- 打包前必须通过 `build_zonkey_exe.bat` 内的 clean rules 检查与 `scripts/release_acceptance.py`。
...
| Windows EXE | `build_zonkey_exe.bat` |
| macOS .app | `./build_zonkey_mac.sh` |
