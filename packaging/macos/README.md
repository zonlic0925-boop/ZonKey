...
3. PyInstaller 生成 `dist/ZonKey.app`
4. 运行 `scripts/release_acceptance.py --app-dir dist/ZonKey.app`
5. 打包 ZIP → `dist_release/ZonKey_macOS_<arch>_<日期>.zip`

## 产物说明

| 路径 | 说明 |
|---|---|
| `dist/ZonKey.app` | 可直接双击运行的 Mac 应用 |
| `dist_release/ZonKey_macOS_*.zip` | 分发给用户的压缩包（含 README） |

**用户数据目录（Mac 冻结版）**：`~/Library/Application Support/ZonKey/`  
...
1. 终端运行：`dist/ZonKey.app/Contents/MacOS/ZonKey` 查看报错  
2. 或浏览器访问：http://127.0.0.1:8765  
3. 查看日志：`~/Library/Application Support/ZonKey/startup_error.log`
...
| | Windows | macOS |
|---|---|---|
| 打包脚本 | `build_zonkey_exe.bat` | `build_zonkey_mac.sh` |
| 产物 | `dist/ZonKey/ZonKey.exe` | `dist/ZonKey.app` |
| 可写数据 | exe 同级目录 | `~/Library/Application Support/ZonKey/` |
