...
export function generatePassword(options: {
  ...
  const avoid = options.avoidAmbiguous ?? options.excludeSimilar ?? false;
  const upper = options.useUpper ?? options.uppercase ?? true;
  const lower = options.useLower ?? options.lowercase ?? true;
  const num = options.useNumbers ?? options.numbers ?? true;
  const sym = options.useSymbols ?? options.symbols ?? false;
  let chars = '';
  if (upper) chars += avoid ? 'ABCDEFGHJKLMNPQRSTUVWXYZ' : 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  if (lower) chars += avoid ? 'abcdefghijkmnpqrstuvwxyz' : 'abcdefghijklmnopqrstuvwxyz';
  if (num) chars += avoid ? '23456789' : '0123456789';
  if (sym) chars += '!@#$%^&*()_+-=[]{}|;:,.<>?';
...
export function evaluatePasswordStrength(password: string): { score: number; label: string; color: string } {
  if (!password) return { score: 0, label: '无', color: 'bg-slate-300' };
  let score = 0;
  ...
  if (score <= 2) return { score, label: '弱', color: 'bg-red-400' };
  if (score <= 4) return { score, label: '中', color: 'bg-yellow-400' };
  return { score, label: '强', color: 'bg-green-400' };
}
