...
import { cropPdfPages, createPdfCroppedFileName } from '../../lib/zonkey/pdfCore'
...
export const PdfCropView: React.FC = () => {
  ...
  const run = async () => {
    ...
    try {
      ...
      const outName = createPdfCroppedFileName(picked.name)
      setOutputBytes(result)
      setOutputName(outName)
      emitTaskDoneBlob(t('tools.pdfCrop'), new Blob([new Uint8Array(result).buffer as ArrayBuffer], { type: 'application/pdf' }), outName)
    } catch (err) {
