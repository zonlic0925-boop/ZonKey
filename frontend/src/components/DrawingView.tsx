...
export const DrawingView: React.FC<DrawingViewProps> = ({ onNotify, backendOnline }) => {
  ...
  return (
    <>
      ...
      />
      <div
        className={`flex-1 w-full h-full flex flex-col lg:flex-row overflow-hidden relative ${isDragging ? 'ring-4 ring-mem-teal ring-inset' : ''}`}
      ...
      >
        <div className="flex flex-col w-full lg:w-80 shrink-0 border-b-[3px] lg:border-b-0 lg:border-r-[3px] border-mem-ink min-h-0 max-h-[42vh] lg:max-h-none">
          <div className="max-h-[140px] lg:max-h-[40%] min-h-[100px] shrink-0 flex flex-col overflow-hidden border-b-2 border-mem-ink/15">
            <FileListDrawer
            files={files}
          ...
          <div className="p-3 bg-white border-t-2 border-mem-ink/15 space-y-3">
            <RedactActionBar
              ...
              onNotify={onNotify}
            />
            <ExportSettingsPanel settings={exportSettings} onChange={setExportSettings} compact onNotify={onNotify} />
          </div>
        </div>
        <CanvasViewport
